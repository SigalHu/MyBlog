...
int main() {
    shared_ptr<A> pa(new A("unique_ptr"));
...
#### 8. 必须判断调用`weak_ptr.lock()`获取的`shared_ptr`的有效性
...
#### 10. 使用`shared_ptr`指向动态数组时，必须使用自定义`deleter`
...
#### 12. 没有保证`shared_ptr`共享的对象的线程安全性
