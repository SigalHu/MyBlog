...
* 该变量在全局数据区分配内存
* ~~静态局部变量在程序执行到该对象的声明处时被首次初始化，即以后的函数调用不再进行初始化~~（此处原作者的说法并不准确，当初始值为常量时，静态局部变量保存在data段，其初始化发生在程序加载时，由系统对其进行初始化，并赋初始值，函数体内的赋初值操作并没有生成相应汇编代码，也就是说一次都没有被执行；当初始值为变量时，静态局部变量保存在bss段，在程序执行到该对象的声明处时被首次初始化，即以后的函数调用不再进行初始化）
