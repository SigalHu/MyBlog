...
&b2.b =0x28ff08
```
虚继承的子类的内存结构和普通继承完全不同。在虚继承的子类中，虚基类按照在子类的虚继承的顺序存放在子类的最后，另外，当普通继承的父类数小于等于1时，在子类的虚函数表中除了存放普通父类虚函数与子类自己的虚函数外，还存放着第一个虚基类在子类内存中偏移字节数的补码，并以一个四个字节的0x00000000来作为分界；当普通继承的父类数大于1时，第一个虚基类在子类内存中偏移字节数的补码以相同形式存放在最后一个普通继承父类的虚函数表中。若存在多个虚基类，则在前一个虚基类的虚函数表中以相同形式存放着下一个虚基类在子类内存中偏移字节数的补码。
```cpp
